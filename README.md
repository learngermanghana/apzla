...
- **Issue a token + link**: `POST /api/checkin-token` with `memberId`, `churchId`, `serviceDate`, optional `serviceType`, `email`, and `baseUrl`.
  ...
  - Returns both the direct link and a `qrImageUrl` (PNG) derived from that link so you can print or display a QR code for attendees to scan.
  - **Per-member tokens**: the token payload stores `memberId`, `churchId`, and `serviceDate`, so you should issue one token per person. A shared link cannot identify multiple members on click; instead, generate individual tokens (looping over your roster) and let the notification queue email each person their own link.
